...
class RandomAffine(object):
    ...
    def __call__(self, *inputs):
        if (self.turn_off_frequency is not None) and (self.frequency_counter % self.turn_off_frequency == 0):
            ...
            tform_matrix = self.transforms[0].transform(inputs[0])
            for tform in self.transforms[1:]:
                tform_matrix = np.dot(tform_matrix, tform.transform(inputs[0]))
        self.frequency_counter += 1 
        return [ apply_transform(X, tform_matrix,
                            fill_mode=self.fill_mode, 
                            fill_value=self.fill_value) for X in inputs]
...
class RandomRotate(object):
    ...
    def __call__(self, *inputs):
        ...
        return [rotate(input, angle, axes=(0, 1), reshape=False) for input in inputs]
...
class RandomTranslate(object):
    ...
    def __call__(self, *inputs):
        ...
        if self.height_range > 0:
            tx = random.uniform(-self.height_range, self.height_range) * inputs[0].shape[0]
        else:
            tx = 0
        # width shift
        if self.width_range > 0:
            ty = random.uniform(-self.width_range, self.width_range) * inputs[0].shape[1]
        else:
            ty = 0
        ...
        if self.lazy:
            ...
            return [apply_transform(X, translation_matrix, 
                                            fill_mode=self.fill_mode, 
                                            fill_value=self.fill_value) for X in inputs]
...
class RandomShear(object):
    ...
    def __call__(self, *inputs):
        ...
        if self.lazy:
            ...
            return [apply_transform(X, shear_matrix, 
                                            fill_mode=self.fill_mode, 
                                            fill_value=self.fill_value)  for X in inputs]
...
class RandomZoom(object):
    ...
    def __call__(self, *inputs):
        ...
        if self.lazy:
            ...
            return [apply_transform(X, zoom_matrix, 
                                            fill_mode=self.fill_mode, 
                                            fill_value=self.fill_value) for X in inputs]
