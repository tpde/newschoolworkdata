...
"""
Callback, TQDM, History, 
ModelCheckpoint, EarlyStopping,
LRScheduler, ReduceLROnPlateau,
CSVLogger, ExperimentLogger, LambdaCallback
"""
...
class TQDM(Callback):
    ...
    def on_train_begin(self, logs):
        ...
        self.num_epochs = logs['num_epochs']

    def on_epoch_begin(self, epoch, logs_epoch=None):
        try:
            self.progbar = tqdm(total=self.num_batches, unit='batches')
            self.progbar.set_description('Epoch %i/%i'%(epoch+1, self.num_epochs))
        except:
            pass

    def on_batch_begin(self, batch, logs_batch=None):
        self.progbar.update(1)

    def on_batch_end(self, batch, logs_batch=None):
        logs = {key:'%.04f' % value for key, value in logs_batch.items()}
        self.progbar.set_postfix(logs)

    def on_epoch_end(self, epoch, logs_epoch=None):
        logs = {key:'%.04f' % value for key, value in logs_epoch.items()}
        self.progbar.set_postfix(logs)
        self.progbar.update()
        self.progbar.close()
...
class History(Callback):
    ...
    def on_train_begin(self, logs=None):
        ...
        self.loss_epoch = {'loss_all':[],
                            'loss_fn':[],
                            'loss_reg':[],
                            'loss_valid':[]}
        self.metric_epoch = {metric+tail:[] for metric in self.metrics for tail in ['_train','_valid']}
        
    def on_epoch_begin(self, epoch, logs=None):
        self.loss_batch = { 'loss_all':0.,
                            'loss_fn':0.,
                            'loss_reg':0.}
        self.num_samples = 0
    
    def on_batch_end(self, batch, logs=None):
        for k in self.loss_batch:
            self.loss_batch[k] = (self.num_samples*self.loss_batch[k] 
                                     + logs[k]*self.batch_size) / (self.num_samples+self.batch_size)
        self.num_samples += self.batch_size

    def on_epoch_end(self, epoch, logs=None):
        for key in self.loss_batch:
            self.loss_epoch[key].append(self.loss_batch[key])
        self.loss_epoch['loss_valid'].append(logs['loss_valid'])
        for key in self.metric_epoch:
            self.metric_epoch[key].append(logs[key])

    def __getitem__(self, name):
        return self.metric_epoch[name]

    def __repr__(self):
        return str(self.metric_epoch)
...
class ModelCheckpoint(Callback):
    ...
    def __init__(self,
        ...
        self.indicator= 0. if maxOrMin in ['max','Max'] else 100000
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if current is None:
            raise('miss monitor in logs!!!')
        else:
            if (self.maxOrMin in ['max','Max'] and current>self.indicator) or (self.maxOrMin in ['min','Min'] and current<self.indicator):
                print('\nEpoch %i: improved from %0.4f to %0.4f saving model to %s' % 
                          (epoch+1, self.indicator, current, self.filepath))
                self.indicator = current
                self.save_checkpoint(epoch, self.filepath)
...
class CSVLogger(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        def handle_value(k):
            is_zero_dim_tensor = isinstance(k, tc.Tensor) and k.dim() == 0
